...
class Trainer(object):
        ...
        if self.opt.text is not None:
            ...
            if 'dreamscene' in self.guidance:
                self.embeddings["dreamscene"]["default"] = {
                    'prompt_embeds': self.guidance["dreamscene"].get_text_embeds([self.opt.text]),
                    'neg_prompt_embeds': self.guidance['dreamscene'].get_text_embeds([self.opt.negative])
                }

            if 'VSD_simple' in self.guidance:
                self.embeddings['VSD_simple']['SD']['default'] = self.guidance['VSD_simple']['SD'].get_text_embeds(
                    [self.opt.text])
                self.embeddings['VSD_simple']['SD']['uncond'] = self.guidance['VSD_simple']['SD'].get_text_embeds(
                    [self.opt.negative])
                self.embeddings['VSD_simple']['clip']['text'] = self.guidance['VSD_simple']['clip'].get_text_embeds(
                    [self.opt.text])
    ...
    def train_step(self, data, save_guidance_path: Path = None):
        ...
        if do_rgbd_loss:
            ...
            # color loss
            gt_rgb = gt_rgb * gt_mask[:, None].float() + bg_color.reshape(B, H, W, 3).permute(0, 3, 1,
                                                                                              2).contiguous() * (
                             1 - gt_mask[:, None].float())
            print(gt_rgb.amin(), gt_rgb.amax())
            exit(0)
        ...
        # novel view loss
            ...
            if 'IF' in self.guidance:
                ...
                loss = loss + self.guidance['IF'].train_step(text_z, pred_rgb, guidance_scale=self.opt.guidance_scale,
                                                             grad_scale=self.opt.lambda_guidance)
            ...
            if 'zero123' in self.guidance:
                ...
                loss = loss + self.guidance['zero123'].train_step(self.embeddings['zero123']['default'], pred_rgb,
            ...
            if "dreamscene" in self.guidance:
                ...
                dist = data["source"]
                loss = loss + self.guidance["dreamscene"].train_step(self.embeddings["dreamscene"]["default"], pred_rgb,
                                                                     pose, intrinsic, dist,
                                                                     guidance_scale=self.opt.guidance_scale,
                                                                     as_latent=as_latent,
                                                                     grad_scale=self.opt.lambda_guidance,
            ...
            if 'clip' in self.guidance:
                ...
                loss = loss + self.guidance['clip'].train_step(self.embeddings['clip'], pred_rgb,
    ...
    def train_one_epoch(self, loader, max_epochs):
        self.log(
            f"==> [{time.strftime('%Y-%m-%d_%H-%M-%S')}] Start Training {self.workspace} Epoch {self.epoch}/{max_epochs}, lr={self.optimizer.param_groups[0]['lr']:.6f} optim: {self.opt.optim} ...")
        # import pdb; pdb.set_trace()
        total_loss = 0
