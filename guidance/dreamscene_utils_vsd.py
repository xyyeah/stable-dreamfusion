...
class DreamScene(nn.Module):
    def __init__(self, device, fp16,
        ...
        self.sd_guide = StableDiffusion(device, fp16, vram_O=True)
    ...
    def _get_sd_noise(self, x_in, t_in, embeddings, guidance_scale):
        with torch.no_grad():
    ...
    def _get_ds_noise(self, x_in, t_in, embeddings, pose, intrinsic, dist, guidance_scale):
        with torch.no_grad():
            noise_preds = []
    ...
    def train_step(self, embeddings, pred_rgb, pose, intrinsic, dist,
        ...
        with torch.no_grad():
            ...
            stand_noise = self._get_sd_noise(x_in, t_in, embeddings, guidance_scale)
            noise_pred = self._get_ds_noise(x_in, t_in, embeddings, pose, intrinsic, dist, guidance_scale)

        w = (1 - self.alphas[t])
        # grad = (grad_scale * w)[:, None, None, None] * (noise_pred - noise)

        grad = (grad_scale * w)[:, None, None, None] * (stand_noise - noise_pred)
