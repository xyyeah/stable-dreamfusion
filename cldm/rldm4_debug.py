...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, noise_level=None, *args, **kwargs):
        ...
        return x, dict(
            c_crossattn=[c.repeat(n_pose, 1, 1)], c_adm=c_adm,
    ...
    def apply_model(self, x_noisy, t, cond, return_rgb=False, *args, **kwargs):
        ...
        x_noisy = self.decode_first_stage(x_noisy)
        print(control.size(), x_noisy.size())
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_full = {"c_crossattn": [uc_cross], 'c_adm': torch.zeros_like(c_adm),
                       'pose': c['pose'], 'intrinsic': c["intrinsic"], 'dist': c['dist']}
            samples_cfg, intermediates = self.sample_log(
                cond={"c_crossattn": [c_cross], 'c_adm': c_adm,
                      'pose': c['pose'], 'intrinsic': c["intrinsic"], 'dist': c['dist']},
                batch_size=N, ddim=use_ddim,
                ddim_steps=ddim_steps, eta=ddim_eta,
...
class ControlNet(nn.Module):
    def __init__(self,
        ...
        self.test_module = MultiFusionModule(hint_channels)
    ...
    def forward(self, x_t, timesteps, context, pose, intrinsic, dist, y):
        # t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        # emb = self.time_embed(t_emb)
        ...
        x = self.test_module(x, pose, intrinsic, dist)
        ...
        #         x = h.chunk(dim=0, chunks=N)
        #         x = fusion(x, pose, intrinsic, dist)
        #         h = torch.cat([x[i].type(self.dtype) for i in range(N)], dim=0)
        #     outs.append(conv(h, emb))
        #
        # h = self.middle_block(h, emb, context)
        # x = h.chunk(dim=0, chunks=N)
        # x = self.middle_fusion_module(x, pose, intrinsic, dist)
        # h = torch.cat([x[i].type(self.dtype) for i in range(N)], dim=0)
        # outs.append(self.middle_block_out(h, emb))
        # print([i.size() for i in outs])
        # exit(0)
        return h
        #
        # H, W = 32, 32
        ...
        for module, conv in zip(self.input_blocks, self.zero_convs):
            if guided_hint:
                ...
                if ref_img is not None:
                    ref_img = module(ref_img, emb, context)
                    x = h.chunk(dim=0, chunks=N)
                    if mask is not None and isinstance(mask, torch.Tensor):
                        mask = mask.chunk(dim=0, chunks=N)
                    x = self.gather_module(x, pose, intrinsic, dist, mask=mask, ref_img=ref_img, emb=emb_tmp)
                    h = torch.cat([x[i].type(self.dtype) for i in range(N)], dim=0)
            else:
                h = module(h, emb, context)
            outs.append(conv(h, emb))
...
class MultiFusionModule(nn.Module):
    ...
    def forward(self, x, pose, intrinsic, dist, emb=None, mask=None, ref_img=None):
        ...
        vol = torch.cat([vol, self.mlp_out(vol)], dim=1)
