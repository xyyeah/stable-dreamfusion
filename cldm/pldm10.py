...
class ControlNet(nn.Module):
    ...
    def forward(self, x, hint, pose, intrinsic, dist, timesteps, context, **kwargs):
        ...
        if x.size(0) != hint.size(0):
            h = torch.cat([x, torch.cat([hint, hint], dim=0)], dim=1).type(self.dtype)
        else:
            h = torch.cat([x, hint], dim=1).type(self.dtype)
        ...
        for module, render in zip(self.input_blocks, self.zero_convs):
            if guided_hint:
                ...
                h = module(h, emb, context)
                # print('Fuck', h.size(), render_hint.size(), context.size())
                # exit(0)
                h = h + render_hint
                guided_hint = False
            else:
...
class ControlLDM(LatentDiffusion):
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        # batch[self.cond_stage_key] = batch[self.cond_stage_key] + batch[self.cond_stage_key]
        # print('Fuck2', batch[self.first_stage_key].size(), len(batch[self.cond_stage_key]))
        # print(batch[self.cond_stage_key].size())
        # exit(0)
        x, c = super().get_input(batch, self.first_stage_key, *args, **kwargs)
        # batch[self.cond_stage_key] = torch.cat([batch[self.cond_stage_key], batch[self.cond_stage_key]], dim=0)
        ...
        return x, dict(c_crossattn=[torch.cat([c, c], dim=0)], c_concat=[control], x_input=batch[self.first_stage_key],
    ...
    def log_images(self, batch, N=4, n_row=2, sample=False, ddim_steps=50, ddim_eta=0.0, return_keys=None,
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            uc_full = {"c_concat": [uc_cat], "c_crossattn": [uc_cross], 'x_input': c['x_input'],
                       'pose': c['pose'], 'intrinsic': c["intrinsic"], 'dist': c['dist']}
            samples_cfg, intermediates = self.sample_log(
                cond={"c_concat": [c_cat], "c_crossattn": [c_cross], 'x_input': c['x_input'],
                      'pose': c['pose'], 'intrinsic': c["intrinsic"], 'dist': c['dist']},
                batch_size=N, ddim=use_ddim,
                ddim_steps=ddim_steps, eta=ddim_eta,
    ...
    def configure_optimizers(self):
        ...
        p1, p2 = {'params': [], 'lr': self.learning_rate}, {'params': [], 'lr': self.learning_rate}
        ...
        if not self.sd_locked:
            p1['params'] += list(self.model.diffusion_model.output_blocks1.parameters())
            p1['params'] += list(self.model.diffusion_model.out.parameters())
...
class Render(nn.Module):
    def __init__(self, hint_channels, time_embed_dim, model_channels, num_samples_per_ray=24, dropout=0.0,
        ...
        #     conv_nd(self.dims, model_channels, model_channels, 1, padding=0),
        #     nn.SiLU(),
        #     conv_nd(self.dims, model_channels, 3, 1, padding=0),
        # )
    ...
    #         positions = positions.reshape(1, Nc, H * W * Np, 3)
    ...
    #     H, W = 64, 64
    #     B = B // 3
    #     positions, ray_samples = self.sample_points(x, pose, intrinsic, dist)  # B, Nc, H * W * Np, 3
    #     Nc = pose.size(1)
    #
    ...
    #         d = torch.stack([rs.frustums.directions for rs in ray_samples], dim=0)  # B, 2 * H * W, Np, 3
    #         d = d.reshape(B, Nc, H * W * Np, 3)
    #         d_encoded = self.direction_encoding(d).permute(0, 3, 1, 2)
    #         plane_features = F.grid_sample(
    #             field, coords, align_corners=True, mode='bilinear'
    #         )  # B, 2, H * W * Np, 3
    #         plane_features = torch.stack(plane_features.chunk(chunks=3, dim=0), dim=1).sum(1)
    #         # print(plane_features.size(), d_encoded.size())
    #         # exit(0)
    ...
    #     density = get_density(density_field, plane_coords)  # B, H * W, Np, 1 -> B, 2, H * W * Np, 1
    #     rgb = get_rgb(color_field, plane_coords).reshape(B, Nc, H * W, Np, -1)  # B, 2, H * W, Np, D
    #     weights = torch.stack([
    #         rs.get_weights(density[i].reshape(Nc * H * W, Np, 1)) for i, rs in enumerate(ray_samples)
    #     ], dim=0).reshape(B, Nc, H * W, Np, 1)  # B, 2, H * W, Np, 1
    #     accumulated_weight = torch.sum(weights, dim=-2)
    #
    #     comp_rgb = torch.sum(weights * rgb, dim=-2)  # B, 2, H * W, D
    #     comp_rgb = comp_rgb + (1.0 - accumulated_weight) * torch.zeros_like(rgb[..., -1, :])
    #     comp_rgb = torch.cat(torch.unbind(comp_rgb, dim=1), dim=0)
    #     feats = comp_rgb.reshape(Nc * B, H, W, self.model_channels).permute(0, 3, 1, 2).contiguous()
    #     rgb = self.map2rgb(feats)
    #     # print('Fuck', input_x.size(), pose.size(), feats.size())
    #     return feats, rgb
